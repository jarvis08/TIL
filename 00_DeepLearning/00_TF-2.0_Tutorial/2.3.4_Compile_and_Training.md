...
이번 챕터에서는 '학습에 사용할 데이터'와 '학습되는 모델'이 준비된 이후, 데이터를 이용하여 모델을 학습시키는 과정에 대해 알아보겠습니다. 학습시키는 과정에는 크게 컴파일(Compile)과 학습(Train)이 있습니다. 컴파일은 `compile` 메서드, 학습은 핏(`fit`)과 핏 제너레이터(`fit_generator`) 메서드를 사용할 수 있습니다.
...
## 핏 제너레이터(fit_generator)를 이용한 학습
